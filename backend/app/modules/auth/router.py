...
from app.modules.auth.permission_models import (
    ALL_SECTIONS,
...
async def me(
    ...
    if current_user.role == UserRole.ADMIN.value:
        data.accessible_sections = list(ALL_SECTIONS)
    else:
        stmt = select(UserAccessibleSection.section_key).where(
            UserAccessibleSection.user_id == current_user.id
        )
        result = await session.execute(stmt)
        stored = list(result.scalars().all())
        data.accessible_sections = stored if stored else list(DEFAULT_AGENT_SECTIONS)
